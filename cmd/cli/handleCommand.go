...
var handlers = map[string]func([]string, *ads.Client){
	...
	"write_array": func(args []string, client *ads.Client) {
		...
		var port uint16 = 350
		if len(args) != 10 {
			fmt.Println("Error: You must provide exactly 10 elements to write to the array.")
			return
		}
		arr := make([]int, 10)
		for i := range 10 {
			...
			if err != nil {
				fmt.Printf("Error: Argument %d ('%s') is not a valid integer.\n", i+1, args[i])
				return
			}
			arr[i] = val
		}
		...
		if err != nil {
			...
			fmt.Printf("Successfully wrote array %v to %s\n", arr, data)
		}
	},
	...
	"write_object": func(args []string, client *ads.Client) {
		...
		for _, arg := range args {
			...
			if len(parts) != 2 {
				fmt.Printf("Error: Argument '%s' must be in key=value format.\n", arg)
				return
			}
		...
		for _, name := range []string{"Ln", "Lv", "PosX", "PosY", "PosZ", "Sct", "cntrNewTryLock"} {
			...
			if !ok {
				fmt.Printf("Error: Missing required field '%s'.\n", name)
				return
			}
			valInt, err := strconv.Atoi(valStr)
			if err != nil {
				fmt.Printf("Error: Field '%s' must be an integer, got '%s'.\n", name, valStr)
				return
			}
			object[name] = valInt
		}
		...
		if !ok {
			fmt.Println("Error: Missing required field 'flgForce'.")
			return
		}
		var flgForce bool
		switch flgForceStr {
		...
		case "false":
			flgForce = false
		default:
			fmt.Println("Error: flgForce must be 'true' or 'false'.")
			return
		}
		...
		if err != nil {
			...
			fmt.Printf("Successfully wrote object %v to %s\n", object, data)
		}
	},
